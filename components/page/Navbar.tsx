...
export default function Navbar() {
    ...
    return (
        <nav className="flex items-center justify-between py-4 px-6 md:px-10">
            ...
            <div className="flex items-center space-x-3">
                ...
                <div className="md:hidden">
                    <Sheet open={isOpen} onOpenChange={setIsOpen}>
                        ...
                        <SheetContent
                            side="top"
                            className="w-full p-6 bg-white text-black rounded-b-2xl shadow-md border-none text-left"
                        >
                            <SheetHeader>
                                <SheetTitle>
                            ...
                            {/* Menu Mobile */}
                            <ul className="flex flex-col space-y-4 text-lg font-semibold mt-2">
                                {NAV_LINKS.map((link) => (
                                    <li key={link.label}>
                                        <a
                                            href={link.href}
            ...
            {/* ==== CENTER : MENU (desktop) ==== */}
            <ul className="hidden md:flex space-x-10 font-bold text-black">
                {NAV_LINKS.map((link) => (
                    <li key={link.label}>
                        <a
                            href={link.href}
