    ...
    return (
        <section className="max-w-7xl mx-auto px-6 py-10">
            ...
            <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-12">
                {categories.map((category) => (
                    <button  key={category}
                        ...
                        className={`px-5 py-2 rounded-full border font-medium text-sm sm:text-base transition-all duration-200 ${activeCategory === category ? "bg-[#fd980e] text-white border-[#fd980e] scale-105" : "border-gray-300 text-gray-700 hover:bg-gray-50"  }`} >
                        {category}
                    </button>
                ))}
            </div>
            ...
            {loading ? (
                ...
                <div  className="grid grid-cols-2 md:grid-cols-4 gap-6 transition-all duration-300 animate-fadeIn"  key={activeCategory}  >
                    {filteredProducts.map((product) => (
                        <div  key={product.id}  className="flex flex-col transform hover:scale-[1.02] transition-transform"  >
